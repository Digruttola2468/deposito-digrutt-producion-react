...
export default function DialogUpdateRemitoNewMercaderia({ open, close }) {
  const { BASE_URL, userSupabase } = useContext(UserContext);
  const { index } = useContext(RemitosContext);
  ...
  const handleUpdate = async () => {
    if (codProducto != null) {
      const idRemito = index.id;
      toast.promise(
        axios
          .put(
            `${BASE_URL}/remito/newProduct/${idRemito}`,
            [
              {
                stock: stock == "" ? 1 : stock,
                price: price == "" ? 0 : price,
