...
import DialogNewOneFacturaNegro from "../dialog/DialogNewOneFacturaNegro";
...
export default function ItemTableNotaEnvio() {
  ...
  const { index, deleteTable, setIndexMercaderia } =
  ...
  return (
    <>
      {data.notaEnvio != null ? (
        <div className="block max-w-[600px] m-auto rounded-lg bg-white p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700 border lg:mx-5 mx-1 mt-2">
      ...
      <DialogNewOneFacturaNegro
        open={dialogNewMercaderia}
        close={() => setDialogNewMercaderia(false)}
      />
      <Dialog
        open={dialogDeleteNotaEnvio}
        onClose={() => setDialogDeleteNotaEnvio(false)}
